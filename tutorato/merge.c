...
int main()
{
    ...
    {
        if (ia < dla && ib < dlb)
        { // ci sono ancora elementi in entrambi gli array a e b che devono essere confrontati.
            if (a[ia] <= b[ib])
            {
                c[ic] = a[ia];
                ia++;
        ...
        else if (ia >= dla && ib < dlb)
        { // tutti gli elementi di a sono già stati copiati in c, ma ci sono ancora elementi in b
            c[ic] = b[ib];
            ib++;
        }
        else if (ia < dla && ib >= dlb)
        { // tutti gli elementi di b sono stati copiati, ma ci sono ancora elementi in a.
            c[ic] = a[ia];
            ia++;
        }
    }
